...
vec3 renderMinecraft(vec2 uv) {
    float xRot = sin(GameTime * 1000. * 0.5) * 0.1 + (3.1415 / 2.);
    float yRot = cos(GameTime * 1000. * 0.5) * 0.1;
    float yCos = cos(yRot);
    float ySin = sin(yRot);
    float xCos = cos(xRot);
    float xSin = sin(xRot);

    vec3 opos = vec3(32.5 + GameTime * 1000. * 6.4, 32.5, 32.5);

    float gggxd = (uv.x - 0.5) * (ScreenSize.x / ScreenSize.y);
...
void main() {
    ...
    fragColor = vec4(renderMinecraft(uv), mask.a);
}