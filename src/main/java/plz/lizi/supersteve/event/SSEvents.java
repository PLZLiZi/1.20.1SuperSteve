	...
	public static class Client {
		...
		public static void bossEventProgress(CustomizeGuiOverlayEvent.BossEventProgress event) {
			...
			if (ss != null) {
				...
				float x = event.getWindow().getGuiScaledWidth() / 2F;
				float y = event.getY();
				float barWidth = 180;
				float barHeight = 22;
		...
		public static void renderLevelStage(RenderLevelStageEvent event) {
			if (event.getStage() == RenderLevelStageEvent.Stage.AFTER_ENTITIES) {
				...
				LevelRenderer zhis = event.getLevelRenderer();
				MultiBufferSource.BufferSource buffer = mc.renderBuffers().bufferSource();
				for (var istc : SSUtil.SS_INSTANCES.values()) {
					SuperSteveEntityBase entity = istc.clientInstance;
					if (entity == null)
						continue;
					if (mc.entityRenderDispatcher.shouldRender(entity, frustum, c0, c1, c2) || entity.hasIndirectPassenger((Entity) (Object) mc.player)) {
					...
					entityrenderer.render(entity, f, partialTick, poseStack, (MultiBufferSource) (Object) buffer, mc.getEntityRenderDispatcher().getPackedLightCoords(entity, partialTick));
					poseStack.popPose();
				}
				buffer.endLastBatch();
